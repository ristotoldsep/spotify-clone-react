...
// This component is looped in TopPlay component
const TopChartCard = ({
  song,
  i,
  isPlaying,
  ...
  <div className="w-full flex flex-row items-center hover:bg-[#4c4264] py-2 p-4 rounded-lg cursor-pointer mb-2">
    ...
    <div className="flex-1 flex flex-row justify-between items-center">
      ...
      <div className="flex-1 flex flex-col justify-center mx-3">
        ...
        <Link to={`/artists/${song?.artists[0].adamid}`}>
          <p className="text-base text-gray-300 mt-1 hover:text-white">{song?.subtitle}</p>
        </Link>
      </div>
    </div>
...
const TopPlay = () => {
  ...
  // Removed dependency from useEffect because we always want to scroll to top when refreshing page
  useEffect(() => {
    divRef.current.scrollIntoView({ behavior: 'smooth' });
  });

  // Take top 5 songs from world charts!
